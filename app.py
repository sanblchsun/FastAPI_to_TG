...
async def receive_message(
    ...
    if not message:
        logger.warning(f"[{timestamp}] ❌ Пустое сообщение от API ключа: {x_api_key}")
        raise HTTPException(status_code=400, detail="Message cannot be empty")

    if len(message) > MAX_MESSAGE_LENGTH:
        logger.warning(f"[{timestamp}] ❌ Превышена длина сообщения от API ключа: {x_api_key} ({len(message)} символов)")
        raise HTTPException(
            status_code=400,
